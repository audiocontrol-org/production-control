...
async function episode(): Promise<string> {
  ...
  // voiceover is declared impure (this fixture always builds it with `FAKE_PROVIDER_MODE=impure`
  // below, and the response must corroborate rather than introduce impurity per FR-012) and
  // podcast is a pure target left unbuilt. The profile is written into the copy so it shadows
  // the shared one.
  const profile = {
    version: 1,
    targets: {
      voiceover: {
        inputs: ['narration'],
        provider: { cmd, impure: { reason: 'fake-provider fixture, impure mode' } },
      },
...
describe('pc readme writes provenance that matches how builds route their bytes', () => {
  it('classifies authored, AI-generated (impure→committed), and reproducible objects', async () => {
    ...
    // The provider's response corroborates the profile's static impure declaration — the README
    // must file it as AI-generated, because that is where its bytes were routed.
