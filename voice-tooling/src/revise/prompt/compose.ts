...
export function buildComposePrompt(input: ModelInput): string {
  ...
  return [
    ...
    '  every numeral in a beat MUST still survive BYTE-EXACT into that beat\'s destination edition',
    '  unit(s), even though the surrounding prose around it is fully rewritten. A quotation lifted',
    '  from a beat keeps its exact wording -- do not paraphrase, reformat, or re-punctuate it.',
