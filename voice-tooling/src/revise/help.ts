...
const COMPOSE_HELP = [
  ...
  '',
  'Fidelity contract (compose) — three guarantee tiers:',
  '',
  'MECHANICAL (deterministically checked):',
  '  - EXPAND, never reproduce: every beat is represented or merged into prose;',
  '    op is NEVER "verbatim", NEVER "cut". No destination unit may be',
  '    byte-identical to the beat it represents (whole-unit copy forbidden).',
  '  - Grounding declared: every edition unit carries exactly one grounding',
  '    record ("grounded" | "connective" | "framing").',
  '  - Payload byte-exact: every citation marker, numeral, and [OPEN-QUESTION]',
  '    marker in a beat survives byte-exact into its destination(s).',
  '',
  'PRODUCER-INSTRUCTION (asked of the model, not mechanically verifiable):',
  '  - Invent nothing: added claims must be attributed, traceable assertions,',
  '    never fabricated facts or sources.',
  '',
  'ADVISORY / NOT CHECKED (reported, not deterministically proven):',
  '  - Spine source fidelity (whether the spine itself cites correctly) —',
  '    fidelity report states spine_source_fidelity: not-checked.',
  '  - Composition semantic grounding (whether composed prose is semantically',
  '    faithful to its beats) — fidelity report states',
