...
- status: planned
- depends-on: design:feature/artifact-adoption
...
## design:feature/artifact-adoption
- status: planned
Generic lifecycle capability to adopt an on-disk artifact that a human edited, without regenerating it. Today a modified artifact can only be rebuilt (destroying the edit) or waived (discarding the fidelity proof), because validate refuses when the on-disk hash does not match the recorded one. Shape: confirm the edit is intentional, run the target's required validators against the edited bytes, record the new output hash and the resulting verdict, keep producer provenance distinct from adoption provenance, require an actor and a reason, and never run the provider. Surfaced by design:feature/voice-editions, where the artifact is prose and an editorial polish pass is a normal part of the lifecycle rather than an edge case; the fidelity validator does not care who wrote the prose, so a human edit can be proven by the same check as machine output once the system can re-attest it. Applies to any target type whose output a human may legitimately revise.